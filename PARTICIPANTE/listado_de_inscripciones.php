...
<div class="wrapper">
  ...
  <div class="card-group">
    ...
    $usuarioId = $usuario_id; // Suponiendo que tienes el ID del usuario actual almacenado en $usuario_id

    // Consultar las inscripciones del usuario actual
    $inscripciones_query = "SELECT e.id, e.nombre, e.descripcion, e.fecha_inicio, e.fecha_fin FROM inscripcion i JOIN evento e ON i.evento_id = e.id WHERE i.usuario_id = '$usuarioId'";
    $inscripciones_result = $conn->query($inscripciones_query);

    if ($inscripciones_result->num_rows > 0) {
      // Mostrar las inscripciones en forma de tabla
      ...
      while ($row = $inscripciones_result->fetch_assoc()) {
        echo '<tr>';
        ...
        echo '<td>';
        echo '<button type="button" class="btn btn-primary btn-imprimir" data-id="' . $row["id"] . '"><i class="fa fa-print"></i> Imprimir</button>';
        echo '</td>';
        echo '</tr>';
      }
      ...
      echo '<p class="card-text">No se encontraron inscripciones.</p>';
      echo '</div>';
      echo '</div>';
    }
...
$(document).ready(function() {
  // Manejar el click en el botón de imprimir
  $(".btn-imprimir").click(function() {
    var eventoId = $(this).data("id");
    var usuarioId = <?php echo $usuario_id; ?>; // Suponiendo que tienes el ID del usuario actual almacenado en $usuario_id

    // Redirigir a la página de impresión con los parámetros
    window.location.href = "imprimir_boleta.php?eventoId=" + eventoId + "&usuarioId=" + usuarioId;
  });
});
</script>
