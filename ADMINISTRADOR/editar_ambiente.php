...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($_FILES['imagen']['name'])) {
    ...
    if (in_array($tipo_archivo, $permitidos)) {
      ...
      move_uploaded_file($temp_archivo, "../imgambientes/$nombre_archivo");

      // Actualizar la imagen del usuario en la base de datos
      $sql_imagen = "UPDATE ambiente SET imagen = '$nombre_archivo' WHERE id = $id";
  ...
  $sql = "UPDATE ambiente SET nombre = '$nombre', aforo_maximo = '$aforo_maximo', tipo = '$tipo', ubicacion = '$ubicacion', estado = '$estado' WHERE id = $id";
  if (mysqli_query($conn, $sql)) {
    ...
    echo "Error al actualizar el ambiente: " . mysqli_error($conn);
  }
}
